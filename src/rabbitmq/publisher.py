...
V2T_VERBAMETRICS_QUEUE = os.getenv("V2T_VERBAMETRICS_QUEUE", "v2t_verbaMetrics")
...
async def publish_results_to_queue(data: dict):
    ...
    try:
        ...
        async with connection:
            ...
            logger.success(f"data published to queue {V2T_RESULTS_QUEUE}: {data}")
    ...
    try:
        ...
        async with connection:
            ...
            await channel.declare_queue(
                V2T_VERBAMETRICS_QUEUE, durable=True, passive=True
            )
            await channel.default_exchange.publish(
                Message(body=json.dumps(data).encode(), delivery_mode=2),
                routing_key=V2T_VERBAMETRICS_QUEUE,
            )
            logger.success(f"data published to queue {V2T_VERBAMETRICS_QUEUE}: {data}")
    except exceptions.AMQPConnectionError as e:
        logger.error(f"failed to connect to RabbitMQ: {e}")
    except Exception as e:
        logger.error(f"error publishing message to RabbitMQ: {e}")
